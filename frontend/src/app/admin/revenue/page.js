...
const RANGES = [{ key: "today", label: "Today" }, { key: "7d", label: "7 Days" }, { key: "30d", label: "30 Days" }];
...
export default function RevenuePage() {
  ...
  // pass-through styles to child components
  const cardStyle = { background: "#161410", border: "1px solid rgba(245,240,232,0.07)", borderRadius: 16, padding: 20, boxShadow: "0 8px 32px rgba(0,0,0,0.3)" };
  const btnStyle = { background: "rgba(255,255,255,0.04)", border: "1px solid rgba(245,240,232,0.08)", color: "#c8bfb0", padding: "8px 16px", borderRadius: 10, cursor: "pointer", fontSize: 13, fontFamily: "inherit", transition: "all 0.2s" };
      ...
      <div style={{ minHeight: "100vh", background: "#0e0e0e", color: "#f5f0e8", padding: "28px 24px", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ marginBottom: 28, animation: "fadeUp 0.4s cubic-bezier(0.16,1,0.3,1)" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
            <div>
              <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 30, fontWeight: 700, color: "#f5f0e8", margin: 0, letterSpacing: -0.5 }}>Revenue Analytics</h1>
              <p style={{ color: "#8a8070", fontSize: 13, margin: "6px 0 0", fontWeight: 300 }}>Track earnings, bills and top-performing items</p>
            </div>

            {/* Range switcher */}
            <div style={{ display: "flex", gap: 4, padding: 4, background: "#161410", border: "1px solid rgba(245,240,232,0.07)", borderRadius: 12 }}>
              {RANGES.map(r => (
                <button key={r.key} className="rv-range-btn" onClick={() => setRange(r.key)}
                  style={{ padding: "8px 18px", borderRadius: 8, border: "none", fontSize: 13, fontWeight: 600, fontFamily: "inherit", cursor: "pointer", transition: "all 0.2s", background: range === r.key ? "#c9a84c" : "transparent", color: range === r.key ? "#0e0e0e" : "#8a8070" }}>
                  {r.label}
                </button>
              ))}
            </div>
        ...
        {/* ERROR */}
        {err && (
          <div style={{ padding: "12px 16px", background: "rgba(239,68,68,0.08)", border: "1px solid rgba(239,68,68,0.2)", borderRadius: 12, color: "#fca5a5", fontSize: 13, marginBottom: 20, display: "flex", gap: 10 }}>
            <span>⚠</span>{err}
          </div>
        )}
        ...
        {loading ? (
          ...
          <div style={{ ...cardStyle, textAlign: "center", padding: "48px 24px" }}>
            ...
            <p style={{ color: "#8a8070", fontSize: 13 }}>Revenue will appear once bills are closed</p>
          </div>
        ) : (
          <>
            {/* STAT CARDS */}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 14, marginBottom: 24 }}>
              {[
                ...
                <div key={s.label} className="rv-stat-card" style={{ background: "#161410", border: `1px solid ${s.border}`, borderRadius: 16, padding: "20px 22px", animation: `countUp 0.4s cubic-bezier(0.16,1,0.3,1) ${i * 0.08}s both` }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                    ...
                    <div style={{ width: 36, height: 36, borderRadius: 10, background: s.accent, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>{s.icon}</div>
                  </div>
                  <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 700, color: s.color, margin: "0 0 6px", letterSpacing: -0.5 }}>{s.value}</p>
                  <p style={{ fontSize: 12, color: "#4a4540", margin: 0 }}>{s.sub}</p>
                </div>
              ))}
            </div>

            {/* CHILD PANELS — pass updated styles */}
