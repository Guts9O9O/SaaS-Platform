...
const EMPTY_FORM = {
  ...
  menuItemVideoLimit: 1,
  restaurantVideoLimit: 2,
  // Admin account
  adminName: "",
  adminEmail: "",
  adminPhone: "",
  ...
  const deleteRestaurant = async (id) => {
    ...
    try {
      const res = await apiFetch(
        `${API}/api/admin/super-admin/restaurants/${id}`,
        { method: "DELETE" }
      );
      if (res.status === 401) return router.replace("/super-admin/login");
      fetchRestaurants();
          ...
          <tbody>
            {restaurants.map((r) => (
              ...
              >
                ...
                <td className="p-4">
                  <span className="px-2 py-1 rounded text-xs bg-neutral-700 text-gray-200">
                    {r.plan || "FREE"}
                  </span>
                </td>
                <td className="p-4">
                ...
                <td className="p-4">
                  <div className="flex gap-2">
                    <button
                      ...
                      className="px-3 py-1.5 rounded bg-blue-600 hover:bg-blue-500 text-white text-xs transition"
                    >
                      View
                    </button>
                    <button
        ...
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-neutral-900 border border-neutral-800 rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div>
                    <label className="block text-xs text-gray-400 mb-1">
                      Slug{" "}
                      <span className="text-gray-600">(leave blank to auto-generate)</span>
                    </label>
                    <input
                      className="w-full p-3 rounded-lg bg-neutral-800 text-white border border-neutral-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      placeholder="e.g. the-grand-spice"
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full p-3 rounded-lg bg-neutral-800 text-white border border-neutral-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      value={form.menuItemVideoLimit}
                      onChange={set("menuItemVideoLimit")}
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-gray-400 mb-1">
                    ...
                    <input
                      ...
                      className="w-full p-3 rounded-lg bg-neutral-800 text-white border border-neutral-700 focus:outline-none focus:ring-2 focus:ring-blue-500"
                      value={form.restaurantVideoLimit}
                      onChange={set("restaurantVideoLimit")}
                    />
                  </div>
                </div>
              </section>
              ...
              <section>
                ...
                <p className="text-xs text-gray-600 mb-3">
                  Optional — fill in to create a login for the restaurant admin right away.
                </p>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
            ...
            <div className="sticky bottom-0 bg-neutral-900 border-t border-neutral-800 px-6 py-4 flex justify-end gap-3">
              <button
                onClick={() => { setShowCreate(false); setForm(EMPTY_FORM); }}
                className="px-4 py-2 rounded-lg bg-neutral-800 hover:bg-neutral-700 text-white text-sm transition"
              >
                Cancel
              </button>
              <button
                onClick={createRestaurant}
                disabled={creating || !form.name.trim() || !form.contact.trim()}
